...
- `App\Models\Category`, `Manufacturer`, `Product`, `ProductImage`, `Cart`,
  `Favorite`, `Order`, `OrderItem` — все с `HasFactory`, у Category/Manufacturer/
  Product ещё `HasSlug` (spatie).
...
Общий прогон `php artisan test` — **123 теста, все зелёные** (не забыть
...
- Заказы (`tests/Feature/OrderTest.php`, 24 теста): доступ к /checkout (пустая

...
- Этап 6 — заказы: оформление заказа из корзины (`/checkout`, гости —
  ...
  выбор способа получения в заказе (самовывоз / доставка по адресу — поле
  `delivery_method`, enum `OrderDeliveryMethod`): доставка пока «в разработке»
  (службы Почта России/СДЭК/Яндекс Доставка/Boxberry/5Post отображаются с
  пометкой «В разработке» и недоступны, зарезервировано поле `delivery_service`),
  оформление и редактирование доступны только самовывозом по адресу магазина
  из настроек (адресные поля nullable, при самовывозе не хранятся),
...
| delivery_method | string(20), default 'pickup' | способ получения: enum `OrderDeliveryMethod` (pickup/delivery) |
| delivery_service | string(60), nullable | служба доставки (зарезервировано; доставка в разработке) |
| delivery_city / delivery_postcode / delivery_address | string, nullable | адрес доставки (хранится только при доставке; при самовывозе — null) |
...
Общий прогон `php artisan test` — **146 тестов, все зелёные** (не забыть
...
- Заказы (`tests/Feature/OrderTest.php`, 47 тестов): доступ к /checkout (пустая
  ...
  **аккаунт гостя из заказа** (форма пароля на success, вход при занятом email,
  ...
  delivery, 404 чужому, валидация, авторизация), **способ получения**
  (доставка отклоняется сервером, самовывоз без адреса, селектор на /checkout,
  delivery_method по умолчанию), **города** (autocomplete на checkout,
...
13. **Доставка по адресу не реализована** — в заказе есть поле
    `delivery_method` и зарезервированное `delivery_service`, на checkout и в
    редактировании блок доставки показан как заготовка (службы Почта России/
    СДЭК/Яндекс Доставка/Boxberry/5Post с пометкой «В разработке»),
    оформление и редактирование принимают только самовывоз. Подключение
    служб доставки, расчёт стоимости и адресные правила — следующий этап.

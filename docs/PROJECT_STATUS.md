...
«Купить»/«В корзину» на количество).
...
Общий прогон `php artisan test` — **88 тестов, все зелёные** (не забыть
`optimize:clear` перед запуском):

- Корзина (`tests/Feature/CartTest.php`, 26 тестов): добавление/обновление/
  ...
  `quantity` в AJAX-ответах, состояние карточки/страницы товара при наличии
  товара в корзине.

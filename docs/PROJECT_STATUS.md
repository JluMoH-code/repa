...
- Этап 6 — заказы: оформление заказа из корзины (`/checkout`, гости —
  ...
  форма редактирования `cabinet/order-edit`), справочник городов России
  (таблица `cities`, 1111 городов, сидер `CitySeeder`) с автопоиском
  (datalist) на поле «Город» в checkout и редактировании, доработана маска
  телефона (любой ввод → +7 (XXX) XXX-XX-XX), исправлено «вечное
  обновление» страницы корзины (`cart.js`: `pageshow` → reload только при
  восстановлении из bfcache, `event.persisted`).
...
12. Справочник городов (`CitySeeder`) — статичный список 1111 городов РФ без
    ...
    таблицей `cities` (город хранится строкой). Автопоиск — браузерный
    `<datalist>`; валидация города мягкая (не обязана совпадать со
    справочником). При необходимости — отдельный JSON-эндпоинт и FK.
